...
const EditarSocio = () => {
  ...
  const [domicilio2, setDomicilio2] = useState('');  // Agregado para domicilio_2
  ...
  const [observacion, setObservacion] = useState('');  // Agregado para observación
  ...
  const obtenerSocio = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3001/obtener_socio.php?nombre=${encodeURIComponent(nombre)}&apellido=${encodeURIComponent(apellido)}`
      );
      ...
      if (socio) {
        ...
        setDomicilio2(socio.domicilio_2 || '');  // Asignación de domicilio_2
        ...
        setObservacion(socio.observacion || '');  // Asignación de observación
      }
  ...
  const guardarSocio = async () => {
    try {
      const response = await fetch('http://localhost:3001/editar_socio.php', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          observacion: observacion || '',  // Enviar observación
          numero: numero || '',
          localidad: localidad || '',
          telefono: telefono || '',
          email: email || '',
          ...
          <div style={styles.inputGroup}>
            <div style={styles.floatingLabelWrapper}>
              <input
                type="text"
                value={domicilio2}
                onChange={(e) => setDomicilio2(e.target.value)} // Maneja el cambio de domicilio_2
                placeholder=" "
                style={styles.input}
                id="domicilio2"
              />
            ...
            <div style={styles.floatingLabelWrapper}>
              <input
                type="text"
                value={observacion}
                onChange={(e) => setObservacion(e.target.value)} // Maneja el cambio de domicilio_2
                placeholder=" "
                style={styles.input}
                id="observacion"
              />
